...
public class LoginAndRegistModel extends BasaManager {
    ...
    public void CodeRequset(String phone, final onRequestComplete listener) {
        AsyncHttp.getInstance().addRequest(new GetSmsRequest(phone, GetSmsRequest.SMSType.REGIST, new Response.Listener<String>() {
            @Override
            public void onResponse(String response) {
                if (listener != null) {
                    listener.onRequestCodeSuccess();
                }
            }
        }, new CodeErrorListener() {
            @Override
